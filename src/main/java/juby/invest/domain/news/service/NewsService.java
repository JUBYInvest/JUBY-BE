...
public class NewsService {
     ...
     */
    public NewsDto.NaverNewsRes callNewsApi(String query){

        for (int attempt = 0; attempt <= MAX_RETRY; attempt++) {
            try {
                NewsDto.NaverNewsRes response = newsSearchRestClient.get()
                        .uri(uriBuilder -> uriBuilder
                                .queryParam("query", query) // 검색어
                                .queryParam("display", DISPLAY_COUNT)
                                .build())
                        .retrieve()
                        .body(NewsDto.NaverNewsRes.class);

                if (response == null){
                    throw new NewsException(NewsErrorCode.NOT_FOUND);
                }

                log.info("네이버 뉴스 API 호출 성공. 종목명:{}, 갯수:{}", query, response.display());
